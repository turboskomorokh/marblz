...
namespace marblz {
  void cursorPosCallback(GLFWwindow *window, double Xpos, double Ypos) {
    if (Events::cursorLocked) {
      Events::deltaX += static_cast<float>(Xpos) - Events::mouseX;
      Events::deltaY += static_cast<float>(Ypos) - Events::mouseY;
    } else {
      Events::cursorStarted = true;
    }
    Events::mouseX = static_cast<float>(Xpos);
    Events::mouseY = static_cast<float>(Ypos);
  }
